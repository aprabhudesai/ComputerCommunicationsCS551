...
void *createBeaconConnector(int *arg)
{
	...
	while(1)
	{
		for(i = 0; i < beaconNodeInfo->no_of_beacons; i++)
		{
			...
			pthread_mutex_lock(&connectedNeighborsMapLock);
			if((connectedNeighborsMapIter = connectedNeighborsMap.find(CONNMAPTYPE::key_type(key))) != connectedNeighborsMap.end())
			{
				pthread_mutex_unlock(&connectedNeighborsMapLock);
				continue;
			}
		...
		{
			//printf("\nNo of neighbors left = %d ; No of Beacon = %d\n",no_of_neighbors_connected_to,beaconNodeInfo->no_of_beacons);
...
void createBeacon(struct NodeInfo *ndInfo)
{
	...
	//pthread_create(&beaconCheckThread,NULL,(void* (*)(void*))checkThread,(void *)ndInfo);
